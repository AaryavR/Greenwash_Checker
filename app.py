...
<style>
    ...
    /* Headers */
    h1, h2, h3 {
        color: #10b981 !important; /* Cyber Green */
    ...
    /* Status Badges */
    .badge-red { background-color: #ef4444; color: white; padding: 2px 6px; border-radius: 4px; font-weight: bold; font-size: 0.8em; }
    .badge-yellow { background-color: #f59e0b; color: black; padding: 2px 6px; border-radius: 4px; font-weight: bold; font-size: 0.8em; }
    .badge-green { background-color: #10b981; color: white; padding: 2px 6px; border-radius: 4px; font-weight: bold; font-size: 0.8em; }
</style>
""", unsafe_allow_html=True)

#Helper to clear vision error if previously set
if "vision_error" not in st.session_state:
...
if uploaded_file is not None:
    if st.button("🚀 INITIATE AUDIT", use_container_width=True):
        ...
        with st.status("Analyzing Visual Data...", expanded=True) as status:
            ...
            data = asyncio.run(vision_engine.extract_data_from_image(uploaded_file))
            
            # Check for Vision Error
            claims_list = data.get("claims", [])
            if claims_list and isinstance(claims_list[0], str) and claims_list[0].startswith("Error reading image"):
                status.update(label="Scanning Failed", state="error")
                st.error(claims_list[0])
                st.stop()

            raw_category = data.get("product_category", "Unknown")
            category = category_engine.identify_category(raw_category)
        ...
        # --- PHASE 2: SCORING & LOGISTICS (Parallel) ---
        with st.spinner("Running Deep Audit..."):
            async def run_calculations():
                # Scoring logic now handles the granular ingredient audit too!
                task_score = scoring_logic.calculate_scores(category, ingredients, claims, origin)
                task_logistics = logistics_check.analyze_logistics(origin)
                return await asyncio.gather(task_score, task_logistics)

            scores, logistics = asyncio.run(run_calculations())
            
            # Phase 3: Sarcasm
            verdict = asyncio.run(sarcasm_bot.get_verdict(scores.get('final_total_score', 50), category, scores.get('breakdown_notes', [])))

        # --- PHASE 3: DASHBOARD DISPLAY ---
        
        # 3.1 Top Banner (Category & Verdict)
        st.divider()
        col1, col2 = st.columns([3, 1])
        with col1:
             st.subheader(f"VERDICT: {verdict}")
        with col2:
             st.metric("Total Score", f"{scores.get('final_total_score', 0)}/100")
             
        # 3.2 The Dashboard (3 Columns)
        st.markdown("### 📊 Sustainability Dashboard")
        
        m1, m2, m3 = st.columns(3)
        with m1:
            st.metric("🌍 Environment (40%)", scores.get('environment_score', 0))
            st.progress(scores.get('environment_score', 0) / 100)
        with m2:
            st.metric("🤝 Social (30%)", scores.get('social_score', 0))
            st.progress(scores.get('social_score', 0) / 100)
        with m3:
            st.metric("⚖️ Governance (30%)", scores.get('governance_score', 0))
            st.progress(scores.get('governance_score', 0) / 100)
            
        # 3.3 Logistics & Breakdown
        st.divider()
        c1, c2 = st.columns(2)
        ...
        with c1:
            ...
            st.metric("Food Miles Penalty", f"{logistics.get('distance_score_adj')} pts")
        ...
        # 3.4 GRANULAR INGREDIENT TABLE (New!)
        st.divider()
        ...
        # 3.5 Raw Data (Removed as requested)
